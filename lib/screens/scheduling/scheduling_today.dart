...
class SchedulingToday {
  ...
  static Map<String, String> alarm = {
    "alarmName": "هشدار",
    "alarmDescription": "زمانبندی پخت غذا",
  };
  ...
  static Future<TimeOfDay?> showPicker(context, Function changeState) async {
    ...
    changeState("todaytime");

    if (timeOfDay != null) {
      int hour = timeOfDay!.hour;
      int minute = timeOfDay!.minute;

      int cHour = currentTime.hour;
      int cMinute = currentTime.minute;

      if (hour * 60 + minute > (cHour * 60 + cMinute) + 5) {
        hour = hour - cHour;
        if (minute < cMinute) {
          minute = (60 - cMinute) + minute;
          if (hour != 0) {
            hour--;
          }
        } else {
        ...
        SnackBarWidget.snackBar("$hour:$minute دیگر باقی مانده است", context);
      }
    } else {
  ...
  static checkTime(Function changeState) {
    if (timeOfDay == null) {
      ...
      changeState("todaytime");
    } else if ((timeOfDay!.hour * 60 + timeOfDay!.minute) <
        (currentTime.hour * 60 + currentTime.minute) + 5) {
      message = "انتخاب اشتباه";
      timeOfDay = null;
      choseTimeState = false;
      changeState("todaytime");
    }
            ...
            Container(
              ...
              child: Column(
                children: [
                        ...
                        child: Row(
                          children: [
                            ...
                            BlocBuilder<SchedulingCubit, SchedulingState>(
                              ...
                              if (state is SchedulingTodayTimeState) {
                  ...
                  BlocBuilder<SchedulingCubit, SchedulingState>(
                      builder: (context, state) {
                    if (state is SchedulingTodayState ||
